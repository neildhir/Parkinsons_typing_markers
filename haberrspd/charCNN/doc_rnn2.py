...
from keras.layers import (LSTM, Bidirectional, Conv1D, Dense, Dropout, Input,
                          Lambda, MaxPooling1D, TimeDistributed)
...
def binarize(x, sz=71):
    return tf.to_float(tf.one_hot(x,
                                  sz,
                                  on_value=1,
                                  off_value=0,
                                  axis=-1))
...
data = pd.read_csv("../../data/test_data/labeledTrainData.tsv",
                   header=0,
                   delimiter="\t",
                   quoting=3)
txt = ''
...
# TODO: Chop up data into train and test (use sklearn function instead)
X_train = X[:20000]
X_test = X[20000:]
...
bi_lstm_sent = Bidirectional(LSTM(128,
...
b_lstm_doc = Bidirectional(LSTM(128,
                                return_sequences=False,
                                dropout=0.15,
                                recurrent_dropout=0.15,
...
# check here that keras is using backend GPU support
model.fit(X_train,
          y_train,
