...
# TODO: load these scores from sklearn instead, this is not good practise
...
def print_f_score(output, target):
    ...
    for label in f.keys():
        cprint("Label: " + c(("  " + str(label))[-5:], 'red') +
               ...
               "F-Score: " + ("  N/A" if f[label]
                              is None else (c("  {:.1f}".format(f[label] * 100)[-5:], "green") + "%"))
               )
    # return p, r, f, _
...
if __name__ == '__main__':
    ...
    precision, recall, TP, TP_plus_FN, TP_plus_FP = precision_recall(
        output.data.numpy().tolist(), target.data.numpy().tolist())
    print('precision')
    print(precision)
    print('recall')
    print(recall)
